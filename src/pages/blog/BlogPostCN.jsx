...
function BlogPostCN() {
  ...
  const post = blogPostsCN.find(p => p.slug === slug);

  if (!post) {
    return (
      <div className="container py-5 text-center">
        <h2>文章不存在</h2>
        <Link to="/cn/blog" className="btn btn-primary mt-3">返回新闻</Link>
      </div>
    );
  }
      ...
      <div className="container py-5">
        <div className="row">
          <div className="col-lg-8 mx-auto">
            ...
            <hr className="my-4" />
            <Link to="/cn/blog" className="btn btn-outline-primary">← 返回新闻</Link>
          </div>
        </div>
      </div>
